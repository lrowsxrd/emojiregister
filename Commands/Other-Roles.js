...
exports.run = async(client, message, args) => {
    let embed = new MessageEmbed().setColor('RANDOM').setFooter('Cross was here.')
    let member = message.mentions.members.first() || message.guild.members.cache.get(args[1])
    let user = message.guild.member(member)
    if (!args[0]) {
        return message.channel.send(embed.setDescription(`${message.author}, Hatalı kullanım örnek: \`.rol vip @Cross/ID\``))
    }
    if (args[0] === "vip") {
